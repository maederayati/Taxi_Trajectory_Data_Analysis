...
a<-seq(0,10000,by=200)
result_rawDest_4plot<-cbind(a,result_rawDest)
save(result_rawDest_4plot, file="RData/result_rawDest_4plot_changeR.RData")
# 
# 
...
names(result_rawDest_4plot)<-c("r", "errorInKm")

png("Graphs/Error_Vs_radious2.png")

p<-ggplot(data=result_rawDest_4plot, aes(x=r, y=errorInKm)) +
  geom_line()+geom_smooth(span = 0.3)+
  #geom_point()+
    labs(title="dissimilarity threshold=0.15  prefix length=10  Average of predictions  no metadata",x="r", y = "Error in KM")
print(p)
dev.off()